...
%\LARGE
 \myfont
 	В ходе применения критерия поглощения возникает, вообще говоря, NP-полная задача определить, является ли полученная ДНФ тождественной единицей: $\mathcal{D}^2 = \bigvee\limits_{i} K^2_i \equiv^{?} 1.$\vspace{20pt}\\
	Тем не менее, в некоторых случаях проверка выполнимости может быть достаточно эффективно решена с помощью алгоритма Блейка построения сокращённой ДНФ.\vspace{20pt}\\
...
\frametitle {\Large \myfont Оценки метода Блейка. Случай одной переменной}
	Общий вид ДНФ:
	$$
		\mathcal{D} = \bigvee_{i = 1}^{t_1}xA_i \vee \bigvee_{i = 1}^{t_2}\bar{x}B_i  \vee \bigvee_iI_i.
	$$
